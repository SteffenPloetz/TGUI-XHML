...
namespace tgui  { namespace xhtml
{
    ...
    bool UtfHelper::checkValid(const std::vector<char>& data, bool utf8)
    {
        ...
        {
            size_t n; // number of bytes, used to encode the code point
            const unsigned char c = static_cast<unsigned char>(data[i]);

            //if (c==0x09 || c==0x0a || c==0x0d || (0x20 <= c && c <= 0x7e) ) n = 0; // is_printable_ascii
            if      (c <= 0x7f)                   n=0; // 0bbbbbbb
            else if (         (c & 0xE0) == 0xC0) n=1; // 110bbbbb
            else if (c == 0xed && i<(ix-1) && (static_cast<unsigned char>(data[i+1]) & 0xa0)==0xa0) return false; //U+d800 to U+dfff
